...
function renderBoard(container) {
  ...
  state.manifest.board.slots.forEach((slot) => {
    ...
    state.manifest.deck.forEach((card) => {
      if (assignedCardIds.has(card.cardId)) {
        return;
      }
...
function assignCard(slotId, cardId) {
  // Prevent assigning the same card multiple times within the same slot.
  const alreadyAssigned = state.manifest.board.assignments.some(
