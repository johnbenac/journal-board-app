...
function loadIntegrations() {
  try {
    ...
    console.warn('Failed to load integrations; using defaults.', err);
    return { espocrm: { ...defaultIntegrations.espocrm } };
  }
}
...
function espocrmBase() {
  ...
  if (!base) throw new Error('EspoCRM base URL not set.');
  return base.replace(/\/+$/, '');
}
...
async function init() {
  ...
  let manifest = null;
  state.integrations = loadIntegrations();
...
function renderDeckFooter(deckContainer) {
  ...
  compareSummary.className = 'compare-summary';
  compareSummary.textContent = count
    ? `${count} selected`
    : 'Select cards to compare';

  const compareActions = document.createElement('div');
  compareActions.className = 'deck-footer-actions';
  ...
  importExport.className = 'deck-footer-imports';
  ...
  importExport.appendChild(importLabel);
  ...
  if (cardTransfer) {
    ...
    importExport.appendChild(importCardLabel);
  }

  footer.appendChild(compareSummary);
  footer.appendChild(compareActions);
...
function showCardModal(card) {
  ...
  if (hasSettings && contactId) {
    ...
    activityBody.appendChild(notesDiv);
